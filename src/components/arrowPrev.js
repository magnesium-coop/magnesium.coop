...
const ArrowPrev = ({ arrow, anchor, fullPageApi }) => {
  if (arrow !== undefined && arrow !== null) {
    return (
      <div
        className={"flex items-center customArrow customArrowPrev my-auto w-8 h-40 md:w-10 md:h-20 lg:w-24 lg:h-48 " + arrow.backgroundColor}>
        <a href={"#" + anchor + "/" + arrow.anchor}
          ...
          <div className="h-full w-full flex flex-wrap lg:flex-no-wrap justify-around align-center content-center">
            <span style={{writingMode: "sideways-lr"}}
                  className={"order-2 lg:order-1 mx-auto my-auto text-sm whitespace-no-wrap uppercase " + arrow.textColor}>{arrow.title}
            </span>
